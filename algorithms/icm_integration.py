...
# stores a single transition batch for ICM training
@dataclass
...
# need to reach robosuite env to query success, go all the way
def unwrap_fully(env):
    while hasattr(env, "env"):
        env = env.env
    return env

# decay function - not fully used since the math is a bit off, leaving for now
...
class ICMIntegration(gym.Wrapper):
    ...
    # check success from the base env
    ...
    # compute intrinsic reward - only one transition !!
    @torch.no_grad()
    ...
    # use Welford's algorithm to normalize the intrinsic reward online
    ...
    # sample batch of transitions from the buffer and make them tensors
    ...
    # perform inline ICM updates using batches from the buffer
    ...
    # reset function part of gym.Env API
    ...
    # step function part of gym.Env API
    def step(self, action: np.ndarray):
        ...
        # baseline path without intrinsic reward
